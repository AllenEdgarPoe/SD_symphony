...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Row():
            txt1 = gr.Textbox(value="a large mountain with red lava floating on it's side and a lot of smoke coming out of it",lines=3)
            txt2 = gr.Textbox(value="a group of rocks in the ocean with a lighthouse on top of them in the distance is a boat in the water", lines=3)
            txt3 = gr.Textbox(value="a large plume of smoke is rising from a large body of water near a city with a bridge and a river", lines=3)
            txt4 = gr.Textbox(value="In the middle of a forest with a huge flame", lines=3)
            txt5 = gr.Textbox(value="a city in the fog with a bridge in the distance and a river running through it with a bridge in the distance", lines=3)
            txt6 = gr.Textbox(value="a field of purple flowers with a mountain in the background with a sunbeam in the sky above it", lines=3)
            txt7 = gr.Textbox(value="a large building with many cars parked in it's parking lot and a highway in the background with a bridge crossing over it", lines=3)
            txt8 = gr.Textbox(value="a group of soldiers standing around a tank in a field with other tanks in the background and a person holding a gun", lines=3)
            txt9 = gr.Textbox(value="a group of people holding hands in front of a fire with a sky background at night time with a large group of people holding hands", lines=3)
            txt10 = gr.Textbox(value="a view of a tall building from across a lake with cherry blossoms in blooming trees in the foreground", lines=3)

        with gr.Row():
            curve = gr.Dropdown(label='Curve function', value='Quick start', choices=[
        ...
        with gr.Row():
            order_frames = gr.Checkbox(label='order_frames', value=False)
        ...
        return [txt1, txt2, txt3, txt4, txt5, txt6, txt7, txt8, txt9, txt10, steps, video_fps, show_images, lead_inout, upscale_meth, upscale_ratio, ssim_min, ssim_diff, ssim_ccrop,
                ssim_diff_min, substep_min, rife_passes, rife_drop, save_stats, order_frames, interpolate_latent, curve, curvestr]
    ...
    def get_prompt_from_texts(self, texts):
    ...
    def run(self, p, txt1, txt2, txt3, txt4, txt5, txt6, txt7, txt8, txt9, txt10, steps, video_fps, show_images, lead_inout, upscale_meth, upscale_ratio, ssim_min, ssim_diff, ssim_ccrop,
        ...
        prompts = self.get_prompt_from_texts(texts)
        if len(prompts)!=0:
                ...
                if len(prompt_images)>0:
                    ...
                    while d<ssim_min:
                        if trial<5:
                            ...
                            p.subseed += random.randint(0,2**32-p.subseed)
                            proc = process_images(p)
                            trials.append(proc)
                            a = transform(prompt_images[-1].convert('RGB')).unsqueeze(0)
                            b = transform(proc.images[0].convert('RGB')).unsqueeze(0)
